...
def _build_sla_terms(data):
    ...
    for test_result in data['result']:
        ...
        if sla:
            terms.update(utils.flatten_dict(sla, test_name + '.'))

    return terms


def _iterate_stats(stats):
    if stats:
        for metric_type, metrics in stats.items():
            for param, value in metrics.items():
                yield (metric_type, param, value)


def _iterate_observed_values(data):
    for test_result in data['result']:
        test_name = (test_result['definition'].get('title') or
                     test_result['definition'].get('class'))
        for iteration_result in test_result['results_per_iteration']:
            cnt = str(len(iteration_result['results_per_agent']))
            for metric_type, param, value in _iterate_stats(
                    iteration_result.get('stats')):
                yield ('.'.join([test_name, cnt, metric_type, param]),
                       value, 'Iteration %s' % cnt)
                yield ('.'.join([test_name, '*', metric_type, param]),
                       value, 'Iteration %s' % cnt)

            for agent_result in iteration_result['results_per_agent']:
                agent_id = agent_result['agent']['id']
                for metric_type, param, value in _iterate_stats(
                        agent_result.get('stats')):
                    yield ('.'.join([test_name, cnt, agent_id, metric_type,
                                     param]),
                           value, 'Agent id %s' % agent_id)
                    yield ('.'.join([test_name, cnt, '*', metric_type, param]),
                           value, 'Agent id %s' % agent_id)


def verify_sla(data, subunit_filename):
    terms = _build_sla_terms(data)
    res = {}
    for item in _iterate_observed_values(data):
        path, value, msg = item
        if path in terms:
            status = value < terms[path]
            res[path] = (status, terms[path])

            if status:
                LOG.debug('SLA "%s = %s" OK on %s', path, value, msg)
            else:
                LOG.debug('SLA "%s = %s" FAILED on %s', path, value, msg)
