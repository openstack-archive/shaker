...
class Deployment(object):
    ...
    def _deploy_support_stacks(self, support_templates, base_dir):
        # By tracking this stack we're saying the current run "owns" this
        # support stack, so it can be deleted later.
        TrackStack = collections.namedtuple('TrackStack', 'name id')
        for template in support_templates:
            for key, value in template.items():
                try:
                    ...
                    current_stack = TrackStack(name=support_name,
                                               id=support_id)
