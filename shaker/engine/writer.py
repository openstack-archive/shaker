...
class ReSTPublisher(object):
    ...
    def add_chart(self, chart_id, chart, meta):
        line_chart = pygal.Line(fill=True, style=style.RedBlueStyle)
        line_chart.x_labels = chart[0][1::10]
    ...
    def add_table(self, table):
        widths = [max(len(c), TABLE_NUMBER_SIGNS) for c in table[0]]
        ...
        for r in table[1:]:
            for i in range(len(widths)):
                ...
                if isinstance(c, numbers.Integral):
                    ...
                    c = '{0:>{1}.{2}}'.format(c, widths[i], TABLE_NUMBER_SIGNS)
                else:
                    c = '{0:<{1}}'.format(c, widths[i])
                self.index.write(c)
                self.index.write('  ')
...
def write_concurrency_block(publisher, all_records, local_records):
    for record in local_records:
        ...
        if len(agent_records) <= 2 and len(local_records) <= 2:
            ...
            # show stats only
            show_stats(publisher, agent_records)
...
def show_stats(publisher, records):
    if len(records) < 1:
        return

    publisher.add_subheader('Stats')
    records.sort(key=lambda x: x['node'])
...
def write_book(doc_folder, data):
    ...
    for scenario in data['scenarios'].keys():
        ...
        for record in test_records:
            ...
            concurrency_records.sort(key=lambda x: x['concurrency'])

            # concurrency_count = len(concurrency_records)
            #
            # if concurrency_count > 0:
            #     if record.get('chart'):
            #         publisher.add_chart(record['id'], record['chart'],
            #                             record['meta'])
